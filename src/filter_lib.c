...
int main(int argc, char *argv[])
{
	...
	char Header[82],Chain,type,ALN_Seq[30];
	char Fasta_Seq[1000];
	char AUX[82];
	float rmsd,score;
	double resolution;
	...
	for(total=0;fscanf(library,"%s\t%c\t%d\t%d\t%s\t%c\t%d\t%d\t%d\t%d\t%lf\t%d\t%f\t%f\n",Header,&Chain,&start1,&length,ALN_Seq,&type,&match_score,&seq_score,&length,&start2,&resolution,&ss_score,&rmsd,&score)!=EOF;total++)
	{
		...
		if(total<5)
			fprintf(out_5,"%s\t%c\t%3d\t%3d\t%s\t%c\t%3d\t%3d\t%3d\t%3d\t%.2lf\t%d\t%.2f\n",Header,Chain,start1,length,ALN_Seq,type,match_score,seq_score,length,start2,resolution,ss_score,rmsd);
	}
